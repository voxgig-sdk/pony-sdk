// Command Pony-cli is a CLI + REPL for the Pony SDK,
...
const entitiesHelp = "character comic episode image kind song"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      PONY_APIKEY (api key), PONY_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: PONY_APIKEY carries the API key and
	// PONY_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("PONY_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("PONY_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewPonySDK(opts)

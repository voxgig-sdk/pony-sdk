...
boru-driven command-line client **and** interactive REPL for the Pony
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pony-cli)
make build
...
./pony-cli list character
./pony-cli load 1 character            # {id:1} shorthand
./pony-cli load '{id:1}' character       # explicit match map
./pony-cli list comic

# 5. Override the API base URL for a single call
PONY_BASE=https://api.example.com ./pony-cli list character
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pony-cli
   ```
...
```sh
./pony-cli load 1 character          # scalar shorthand for {id:1}
./pony-cli load '{id:1}' character     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `pony>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/pony-cli`. |
...
(`./pony-cli <expr>`) and interactively (the REPL), and expressions compose the

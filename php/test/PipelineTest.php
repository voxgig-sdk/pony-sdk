...
{
    private static function utility(): PonyUtility
    {
        return new PonyUtility();
    }
    ...
    private static function ctx(array $over = []): PonyContext
    {
        ...
        $ctx->op = new PonyOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof PonyError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new PonyOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = PonyMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new PonyOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = PonyMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new PonyOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = PonyMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PonySpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = PonyMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new PonyResponse([]),
            'result' => new PonyResult([]),
        ]);
        [, $err] = PonyMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new PonySpec(['step' => 's']),
            'response' => new PonyResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new PonyResult([]),
        ]);
        [, $err] = PonyMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new PonySpec(['step' => 's']),
            'response' => new PonyResponse(self::resp(200, ['v' => 1])),
            'result' => new PonyResult([]),
        ]);
        [, $err] = PonyMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new PonySpec(['step' => 's']),
            'response' => new PonyResponse(self::resp(200, ['v' => 2])),
            'result' => new PonyResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        PonyMakeResponse::call($ctx);
    ...
    {
        $preset = new PonyResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new PonySpec([]),
            'response' => new PonyResponse([]),
            'result' => new PonyResult([]),
        ...
        [$got, $err] = PonyMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new PonyResult([])]);
        [, $err] = PonyMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new PonySpec([]), 'result' => null]);
        [, $err] = PonyMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PonySpec(['step' => 's']),
            'result' => new PonyResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new PonyOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PonyMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PonySpec(['step' => 's']),
            'result' => new PonyResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new PonyOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PonyMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PonyResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new PonySpec([]),
            'result' => new PonyResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = PonyMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new PonyError('boom', 'boom');
        $utility->fetcher = function (PonyContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PonySpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = PonyMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (PonyContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = PonyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(PonyResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (PonyContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PonySpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = PonyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new PonyResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new PonySpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = PonyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = PonyMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new PonyResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, PonyDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new PonyResult(['ok' => false])]);
        $this->expectException(PonyError::class);
        PonyDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new PonyResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', PonyMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new PonyBaseFeature();
        $b = new PonyBaseFeature();
        PonyFeatureAdd::call($ctx, $a);
        PonyFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): PonyBaseFeature
    {
        $f = new PonyBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): PonyContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new PonySpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new PonyResponse(['headers' => null]),
            'result' => new PonyResult([]),
        ]);
        PonyResultHeaders::call($ctx);

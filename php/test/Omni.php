...
//    only - a typed PonyContext there would make every ctx
...
final class PonyOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new PonyContext($clean, null);
    ...
    public static function err_from_map($m): ?PonyError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new PonyError(is_string($code) ? $code : '', $msg);
    }

...
MCP server exposing the Pony SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `pony_list` | `entity`, optional `query` map | First page of records as JSON |
| `pony_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `pony_list` and
`pony_load` tools become available in new sessions.

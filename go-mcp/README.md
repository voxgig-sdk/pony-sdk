...
[MCP](https://modelcontextprotocol.io) server exposing the Pony SDK as
two agent tools — `pony_list` and `pony_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pony-mcp)
make build

# 2. Provide credentials via the environment
export PONY_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user pony \
  -- /absolute/path/to/pony-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./pony-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// pony_list: first page of records
{ "entity": "character" }
{ "entity": "character", "query": { } }

// pony_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pony-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user pony \
     -- "$PWD"/dist/*/pony-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `pony_list` and `pony_load` tools now appear
   in new sessions. Ask the agent to *"list character using pony"*
   and it calls `pony_list` with `{"entity":"character"}`.
...
```sh
export PONY_APIKEY=sk_live_xxx            # API key
export PONY_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `pony_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `pony_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PONY_APIKEY` | API key sent with every request. |
| `PONY_BASE` | Optional override of the API base URL. |

### Entities

The 6 entities valid as the `entity` argument:

character | comic | episode | image | kind | song

### Smoke test via HTTP (raw JSON-RPC)

```sh
./pony-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"pony_load","arguments":{"entity":"character","query":{"id":1}}}}'
```

...
type Args struct {
	Entity string         `json:"entity" jsonschema:"character | comic | episode | image | kind | song"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.PonySDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "pony_list",
		Description: "List records from Pony. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "pony_load",
		Description: "Load a single record from Pony. " +
...
func runOp(client *sdk.PonySDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.PonySDK, name string) (sdk.PonyEntity, error) {
	switch strings.ToLower(name) {
	case "character":
		return client.Character(nil), nil
	case "comic":
		return client.Comic(nil), nil
	case "episode":
		return client.Episode(nil), nil
	case "image":
		return client.Image(nil), nil
	case "kind":
		return client.Kind(nil), nil
	case "song":

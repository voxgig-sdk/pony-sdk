...
The TypeScript SDK for the Pony API — a type-safe, entity-oriented client with full async/await support.
...
const comic = await client.Comic().list()
// comic is the entity, populated with mock response data
// — call comic.data() for the record itself
console.log(comic)
```
...
| `tester(testopts?, sdkopts?)` | `PonySDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `PonySDK.test(testopts?, sdkopts?)` | `PonySDK` | Create a test-mode client. |
...
| `client` | `client(): PonySDK` | Return the parent SDK client. |
...
// comic.data() now returns the comic data from the last `list`
// comic.match() returns the last match criteria
```

...
//     (\`new PonySDK(...)\` / \`PonySDK.test(...)\` become
//     \`PonySDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"character":{"test01":{"id":"test01"}},"comic":{"test01":{"id":"test01"}},"episode":{"test01":{"id":"test01"}},"image":{"test01":{"id":"test01"}},"kind":{"test01":{"id":"test01"}},"song":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new PonySDK(...)\` and any
// \`PonySDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+PonySDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/PonySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

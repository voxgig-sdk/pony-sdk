...
(0, node_test_1.describe)('SongDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-song', async (t) => {
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.episode ?? listArr[0]?.id;
            if (null == candidateId) {
                return; // skip: list response shape does not expose load identifier
            }
            params.episode = candidateId;
    ...
    (0, node_test_1.test)('direct-list-song', async (t) => {
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-song', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['PONY_TEST_SONG_ENTID'];

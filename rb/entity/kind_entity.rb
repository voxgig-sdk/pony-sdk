...
class KindEntity
  ...
  def make
    opts = @_entopts.dup
    KindEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = PonyHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Kind, Hash] the current Kind data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = PonyHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Kind fields)
  def match_get
  ...
  # Load a single Kind.
  #
  # @param reqmatch [KindLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Kind.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Kind, Hash] the loaded Kind; raises PonyError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = PonyHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [KindListMatch, Hash, nil] match filter (any subset of
  #   Kind fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Kind>, Array] the matching Kind items; raises PonyError on failure

...
```lua
local results, err = client:Comic(nil):list(nil, nil)
```
...
```lua
local result, err = client:Comic(nil):load({ id = "comic_id" }, nil)
```
...
```lua
local results, err = client:Kind(nil):list(nil, nil)
```
...
```lua
local result, err = client:Kind(nil):load({ id = "kind_id" }, nil)
```

...
describe("KindDirect", function()
  it("should direct-list-kind", function()
    local setup = kind_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-kind", setup.live and "live" or "unit")
  ...
  it("should direct-load-kind", function()
    local setup = kind_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-kind", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end

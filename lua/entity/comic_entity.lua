...
local ComicEntity = {}
ComicEntity.__index = ComicEntity


function ComicEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ComicEntity)
  self._name = "comic"
  self._client = client
...
function ComicEntity:get_name()
  return self._name
end


function ComicEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ComicEntity.new(self._client, opts)
end
...
function ComicEntity:mark_deleted()
  self._deleted = true
end


function ComicEntity:deleted()
  return true == self._deleted
end


function ComicEntity:data_set(args)
  if args ~= nil then
...
function ComicEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ComicEntity:match_set(args)
  if args ~= nil then
...
function ComicEntity:match_get()
...
function ComicEntity:stream(action, args, callopts)
...
function ComicEntity:_run_op(ctx, post_done)
  local utility = self._utility

...
func (sdk *PonySDK) OptionsMap() map[string]any {
...
func (sdk *PonySDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *PonySDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *PonySDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *PonySDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *PonySDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *PonySDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("PonySDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *PonySDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *PonySDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("PonySDK: graphql: %s", msg)
		res["graphql"] = errors
	}

...
import (
	...
	omni "github.com/voxgig-sdk/pony-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PonyError); is {
				out := map[string]any{
					"name":    "PonyError",
...
func omniCtx(arg any, client *sdk.PonySDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PonySDK); is {
			client = live

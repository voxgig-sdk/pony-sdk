...
func TestKindDirect(t *testing.T) {
	t.Run("direct-list-kind", func(t *testing.T) {
		setup := kindDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-kind", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-kind", func(t *testing.T) {
		setup := kindDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-kind", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type kindDirectSetupResult struct {
	client *sdk.PonySDK
...
func kindDirectSetup(mockres any) *kindDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"PONY_TEST_KIND_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["PONY_TEST_KIND_ENTID"]; ok {
		...
		return &kindDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &kindDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}

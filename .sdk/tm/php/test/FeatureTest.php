...
class FtCtrl extends PonyControl
{
...
class FtClient
{
    ...
    private PonyUtility $utility;

    public function __construct(PonyUtility $utility, array $options)
    {
    ...
    public function get_utility(): PonyUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PonyContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PonyContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PonyConfig::make_feature($name);
    ...
    private static function buildUrl(PonySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PonyError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PonyResponse($fetched);
        }
    ...
    private function fail(PonyContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PonyContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
